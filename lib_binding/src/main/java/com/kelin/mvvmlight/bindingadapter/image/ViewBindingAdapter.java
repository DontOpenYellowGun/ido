...
//        }
    ...
    @BindingAdapter(value = {"url"})
    public static void setImageUrl(final ImageView imageView, Uri url) {
        Glide.with(imageView.getContext()).load(url).into(imageView);
    }

    @BindingAdapter(value = {"url", "placeholderImageRes"})
    public static void setImageUrl(final ImageView imageView, Uri url, Drawable placeholderImageRes) {
