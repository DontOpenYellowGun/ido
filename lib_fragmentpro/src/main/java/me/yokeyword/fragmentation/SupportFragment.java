...
public class SupportFragment extends RxFragment implements ISupportFragment {
    ...
    public void loadRootFragment(int containerId, String fragmentPath) {
        SupportFragment toFragment = (SupportFragment) ARouter.getInstance().build(fragmentPath).navigation();
        mDelegate.loadRootFragment(containerId, toFragment);
    }

    public void loadRootFragment(int containerId, String fragmentPath, Bundle arg) {
        SupportFragment toFragment = (SupportFragment) ARouter.getInstance().build(fragmentPath).navigation();
        if (toFragment != null) {
    ...
    public void start(String fragmentPath, Bundle arg) {
        SupportFragment toFragment = (SupportFragment) ARouter.getInstance().build(fragmentPath).navigation();
        if (toFragment != null) {
            toFragment.setArguments(arg);
            mDelegate.start(toFragment, STANDARD);
        } else {
            throw new RuntimeException("can't find your toFragment,please check out your router path!");
        }
    }
